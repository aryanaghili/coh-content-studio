...
export async function generate(config, input) {
  ...
  let rawText;
  try {
    rawText = await callAnthropic(apiKey, model, baseUrl, systemPrompt, prompt, true);
  } catch (err) {
  ...
  if (parsed && parsed.success === false && parsed.errorType === 'AI_PARSE_ERROR') {
      try {
          ...
          const repairedText = await callAnthropic(apiKey, model, baseUrl, systemPrompt, repairPrompt, true);
          const repairedParsed = repairJson(repairedText);
          if (repairedParsed && repairedParsed.success !== false) {
              parsed = repairedParsed;
          }
      } catch (e) {
          console.error("JSON Repair retry failed for anthropic", e);
      }
  }
  ...
  if (!parsed) {
     return { success: false, errorType: 'AI_PARSE_ERROR', userMessage: 'Could not parse response from anthropic.', fallbackText: rawText };
  }
