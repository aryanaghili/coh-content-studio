    ...
    <div className="h-full flex flex-col bg-[#faf9f6]">
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {calendars.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center p-12 bg-white border border-dashed border-coh-gold/40 rounded">
              <Calendar size={48} className="text-coh-gold/30 mb-4" />
              <p className="text-coh-navy/60 font-medium">No saved calendars yet.</p>
              <p className="text-xs text-coh-navy/40 mt-1">Generate and save calendars in the Studio.</p>
            </div>
          ) : (
            calendars.map(cal => (
              <div key={cal.id} className="bg-white border border-coh-gold/20 p-5 rounded shadow-sm hover:shadow transition flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-bold text-coh-navy font-serif text-lg">{cal.title}</h3>
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${
                      cal.status === 'Approved' ? 'bg-green-100 text-green-800' :
                      cal.status === 'Archived' ? 'bg-gray-100 text-gray-600' :
                      'bg-yellow-100 text-yellow-800'
                    }`}>
                      {cal.status}
                    </span>
                  </div>
                  <div className="space-y-1 text-xs text-coh-navy/70 mb-4">
                    <p><span className="font-bold text-coh-navy">Month:</span> {cal.planningMonth} {cal.planningYear} (v{cal.versionNumber})</p>
                    <p><span className="font-bold text-coh-navy">Primary Focus:</span> {cal.primaryStrategicFocus}</p>
                    <p><span className="font-bold text-coh-navy">Audience:</span> {cal.primaryAudience}</p>
                    <p><span className="font-bold text-coh-navy">Items:</span> {cal.calendarItems.length} ({cal.approvedItemCount} approved)</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 pt-4 border-t border-coh-gold/10">
                  ...
                  <Button variant="outline" onClick={() => handleDelete(cal.id)} className="text-xs py-1 px-2 text-red-600 border-red-200 hover:bg-red-50">
                    <Trash2 size={14} />
                  </Button>
                  <select 
                    className="flex-1 bg-white border border-coh-gold/20 text-coh-navy text-[10px] font-semibold px-1 py-1 rounded outline-none cursor-pointer hover:border-coh-gold/40 transition"
                    onChange={(e) => {
                      if (e.target.value) {
