...
# 2. Remove Open Operating Core Button
...
# 3. Remove "Suggested Core Documents" section completely
...
# 5. Remove Role and Supports Section badges from Source List rendering
role_badge_regex = r'\{source\.role && source\.role !== \'Task Source\' && \(\s*<span className="text-\[9px\] bg-blue-50 text-blue-700 px-1\.5 py-0\.5 rounded font-bold uppercase border border-blue-200">\s*\{source\.role\}\s*</span>\s*\)\}'
content = re.sub(role_badge_regex, '', content, flags=re.DOTALL)

supports_badge_regex = r'\{source\.supportsOperatingCoreSection && source\.supportsOperatingCoreSection !== \'None\' && \(\s*<span className="text-\[9px\] bg-purple-50 text-purple-700 px-1\.5 py-0\.5 rounded font-bold uppercase border border-purple-200">\s*Supports: \{source\.supportsOperatingCoreSection\}\s*</span>\s*\)\}'
content = re.sub(supports_badge_regex, '', content, flags=re.DOTALL)

# Remove any remaining mentions of source.role in conditionals
content = re.sub(r's\.role === \'Core Document\'', 'false', content)
content = re.sub(r'sourceLibraryFilter === \'Core Documents\'', 'false', content)

# Remove the count logic for Core Documents
content = re.sub(r'const coreDocumentsCount = [^;]+;', '', content)

with open('src/App.tsx', 'w') as f:
    f.write(content)
print("App.tsx UI updated")
