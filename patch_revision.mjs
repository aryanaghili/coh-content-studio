...
// 1. Add imports
const importStr = "import { REVISION_ACTIONS, REVISION_GROUP_ORDER } from './lib/revisionActions';\n";
if (!content.includes('REVISION_ACTIONS')) {
    content = content.replace("import { DEFAULT_COH_SOURCES } from './data/defaultSources';", importStr + "import { DEFAULT_COH_SOURCES } from './data/defaultSources';");
}

// 2. Fix applyRevision
const applyRevisionOld = `          revised = result.revisedCopy || revised;
...
const applyRevisionNew = `          revised = result.revisedCopy || revised;
          actionLabel = action === 'custom-instruction' ? \`Custom: \${customRevisionInstruction || 'Rewrite'}\` : actionLabel;
...
content = content.replace(applyRevisionOld, applyRevisionNew);

// 3. Fix Layout
// Let's locate the entire `activeTab === 'revision-studio'` block
const startIdx = content.indexOf("{activeTab === 'revision-studio' && (");
const endIdx = content.indexOf("{activeTab === 'visual-studio' && (");

if (startIdx !== -1 && endIdx !== -1) {
    // Generate the new Revision Studio block
    const newRevisionStudioBlock = `{activeTab === 'revision-studio' && (
          <div className="space-y-6 animate-fadeIn max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              {/* Right Column: Revision Controls (always rendered) */}
              <div className="lg:col-span-5 xl:col-span-4 bg-white border border-coh-gold/20 p-5 rounded shadow-sm h-fit max-h-[85vh] overflow-y-auto space-y-5">
                <div>
                  ...
                  {!activeDraftText && (
                    <p className="text-xs text-coh-navy/60 font-semibold mb-2">Paste or import content first to enable controls.</p>
                  )}
                </div>

                <div className={\`space-y-4 \${!activeDraftText ? 'opacity-50 grayscale pointer-events-none' : ''}\`}>
...
`;
    ...
    content = before + newRevisionStudioBlock + "\n        " + after;
} else {
    console.error("Could not find start/end indices for revision studio block");
}
